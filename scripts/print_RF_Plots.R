...
# name procedures
proc <- c('alr', 'clr', 'ilr', 'log', 'tss', 'heilinger')

# run full process to generate figures by study
for (name in study) {
  # read in all Random Forest output files
  alr <- read_csv(paste0(name,"_",proc[1],'_table.csv'))
  clr <- read_csv(paste0(name,"_",proc[2],'_table.csv'))
  ilr <- read_csv(paste0(name,"_",proc[3],'_table.csv'))
  log <- read_csv(paste0(name,"_",proc[4],'_table.csv'))
  log <- read_csv(paste0(name,"_",proc[4],'_table.csv'))
  log <- read_csv(paste0(name,"_",proc[4],'_table.csv'))
